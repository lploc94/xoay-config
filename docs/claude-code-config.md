...
| [**CLAUDE.md**](http://CLAUDE.md) | `~/.claude/CLAUDE.md` | `CLAUDE.md` or `.claude/CLAUDE.md` | `CLAUDE.local.md` |
...
- **User settings** are defined in `~/.claude/settings.json` and apply to allprojects.
- **Project settings** are saved in your project directory:
  - `.claude/settings.json` for settings that are checked into source control and shared with your team
  - `.claude/settings.local.json` for settings that are not checked in, useful for personal preferences and experimentation. Claude Code will configure git to ignore `.claude/settings.local.json` when it is created.
- **Managed settings**: For organizations that need centralized control, Claude Code supports multiple delivery mechanisms for managed settings. All use the same JSON format and cannot be overridden by user or project settings:See [managed settings](/en/permissions#managed-only-settings) and [Managed MCP configuration](/en/mcp#managed-mcp-configuration) for details.<Note>Managed deployments can also restrict **plugin marketplace additions** using`strictKnownMarketplaces`. For more information, see [Managed marketplace restrictions](/en/plugin-marketplaces#managed-marketplace-restrictions).</Note>
  - **Server-managed settings**: delivered from Anthropic's servers via the [Claude.ai](http://Claude.ai) admin console. See [server-managed settings](/en/server-managed-settings).
...

{
  ...
  "companyAnnouncements": [
    ...
    "Welcome to Acme Corp! Review our code guidelines at [docs.acme.com](http://docs.acme.com)",
...
The `$schema` line in the example above points to the [official JSON schema](%5Bhttps://json.schemastore.org/claude-code-settings.json%5D(https://json.schemastore.org/claude-code-settings.json)) for Claude Code settings. Adding it to your `settings.json` enables autocomplete and inline validation in VS Code, Cursor, and any other editor that supports JSON schema validation.
...
| `forceLoginMethod` | Use `claudeai` to restrict login to [Claude.ai](http://Claude.ai) accounts, `console` to restrict login to Claude Console (API usage billing) accounts | `claudeai` |
...
| `WebFetch(domain:example.com)` | Matches fetch requests to [example.com](http://example.com) |
...
| `network.allowLocalBinding` | Allow binding to [localhost](http://localhost) ports (macOS only). Default: false | `true` |
...

{
  ...
  "sandbox": {
    ...
    "network": {
      ...
      "allowedDomains": ["[github.com](http://github.com)", "*.[npmjs.org](http://npmjs.org)", "[registry.yarnpkg.com](http://registry.yarnpkg.com)"],
...
- Commits use [git trailers](%5Bhttps://git-scm.com/docs/git-interpret-trailers%5D(https://git-scm.com/docs/git-interpret-trailers)) (like `Co-Authored-By`) by default, which can be customized or disabled
...

{
  ...
  "attribution": {
    ...
    "commit": "Generated with AI\n\nCo-Authored-By: AI <[ai@example.com](mailto:ai@example.com)>",
...

{
  ...
  "fileSuggestion": {
    ...
    "command": "~/.claude/[file-suggestion.sh](http://file-suggestion.sh)"
...

{
  ...
  "allowedHttpHookUrls": ["[https://hooks.example.com/*](https://hooks.example.com/*)", "[http://localhost:*](http://localhost:*)"]
  ...
  "extraKnownMarketplaces": {
    ...
    "security-plugins": {
      ...
      "source": {
        ...
        "url": "[https://git.example.com/security/plugins.git](https://git.example.com/security/plugins.git)"
...
{ "source": "git", "url": "[https://gitlab.example.com/tools/plugins.git](https://gitlab.example.com/tools/plugins.git)" }

{ "source": "git", "url": "[https://bitbucket.org/acme-corp/plugins.git](https://bitbucket.org/acme-corp/plugins.git)", "ref": "production" }

{ "source": "git", "url": "ssh://[git@git.example.com](mailto:git@git.example.com)/plugins.git", "ref": "v3.1", "path": "approved" }
...
{ "source": "url", "url": "[https://plugins.example.com/marketplace.json](https://plugins.example.com/marketplace.json)" }

{ "source": "url", "url": "[https://cdn.example.com/marketplace.json](https://cdn.example.com/marketplace.json)", "headers": { "Authorization": "Bearer  
...

{
  ...
  "strictKnownMarketplaces": [
...

    {
      ...
      "url": "[https://plugins.example.com/marketplace.json](https://plugins.example.com/marketplace.json)"
...
| `AWS_BEARER_TOKEN_BEDROCK` | Bedrock API key for authentication (see [Bedrock API keys](%5Bhttps://aws.amazon.com/blogs/machine-learning/accelerate-ai-development-with-amazon-bedrock-api-keys/%5D(https://aws.amazon.com/blogs/machine-learning/accelerate-ai-development-with-amazon-bedrock-api-keys/))) | |
...
| `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD` | Set to `1` to load [CLAUDE.md](http://CLAUDE.md) files from directories specified with `--add-dir`. By default, additional directories do not load memory files | `1` |
...
| `CLAUDE_CODE_SIMPLE` | Set to `1` to run with a minimal system prompt and only the Bash, file read, and file edit tools. Disables MCP tools, attachments, hooks, and [CLAUDE.md](http://CLAUDE.md) files | |
...
export CLAUDE_ENV_FILE=/path/to/[env-setup.sh](http://env-setup.sh)
